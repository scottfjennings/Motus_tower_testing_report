...
## Import drone telemetry data then calculate the mean possition for each second.
...
# First generate df representing start and end times for each drone duplicated for each antenna
...
flights_antennae_detections <- all_tags_tower %>% 
  ...
  filter(between(ts, start.time, end.time)) %>% 
...
# join tag detections with drone telemetry
